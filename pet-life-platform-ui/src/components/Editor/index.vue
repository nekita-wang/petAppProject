<template>
  <div>
    <el-upload :action="uploadUrl" :before-upload="handleBeforeUpload" :on-success="handleUploadSuccess"
      :on-error="handleUploadError" name="avatarfile" :show-file-list="false" :headers="headers" style="display: none"
  ...
  methods: {
    ...
    handleUploadSuccess(res, file) {
      ...
      if (res.code == 200) {
        ...
        // 适配新的返回格式：从 res.data 中获取文件信息
        const uploadData = res.data || res;
        const fileName = uploadData.fileName || uploadData.url || res.fileName;
        // 插入图片  fileName为服务器返回的图片地址
